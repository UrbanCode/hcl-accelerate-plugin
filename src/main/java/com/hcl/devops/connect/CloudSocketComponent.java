 ...
 * (c) Copyright HCL Technologies Ltd. 2018, 2019, 2020, 2021. All Rights Reserved.. All Rights Reserved.
 *
...
public class CloudSocketComponent {
    ...
    public void connectToCloudServices(Entry entry) throws Exception {
        if (entry.isConfigured()) {
            connectToAMQP(entry);
            log.info("[HCL Accelerate " + entry.getBaseUrl() + "] CloudSocketComponent#connectToCloudServices "
    ...
    public void connectToAMQP(Entry entry) throws Exception {
        ...
        String host = em.getAccelerateHostname(entry);
                        ...
                        if (entry.getCheckDuplicate() == true) {
                            ...
                            if (buildDetailsObject.has("builds")) {
                                ...
                                boolean isFound = allConsoleLogs
                                        .contains("Started due to a request from HCL Accelerate. Work Id: " + workId);
                                if (isFound == true) {
                                    log.info(
    ...
    public static boolean checkQueueAvailability(Channel channel, String queueName, Entry entry) throws IOException {
        try {
            ...
            log.error("[HCL Accelerate " + entry.getBaseUrl() + "] Checking Queue availability threw exception: ",e);
        }
        return false;
    }

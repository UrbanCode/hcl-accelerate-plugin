...
public class UploadDeployment extends Builder implements SimpleBuildStep {
    ...
    throws AbortException, InterruptedException, IOException {
        ...
        List<Entry> finalEntriesList = CloudPublisher.getFinalEntriesList("Upload Deployment", instanceBaseUrl, getEntries());
        ...
        if (initiator != null && !initiator.equals("")) {
            ...
            for (Cause cause : build.getCauses()) {
                if (cause instanceof UserIdCause) {
                    ...
                    payload.put("by_user", "Upstream job \"" + upstreamCause.getUpstreamProject() + "\", build \"" 
        ...
        for (Entry entry : finalEntriesList) {
            ...
            listener.getLogger().println("Uploading deployment \"" + payload.get("version_name") + "\" of \""
                    + payload.get("name") + "\" to HCL Accelerate (" + entry.getBaseUrl() + ").");
            try {
                if (!entry.isConfigured()) {
                    listener.getLogger().println(
                            "Could not upload deployments to Accelerate as there is no configuration specified.");
                    return;
                }
                String response = CloudPublisher.uploadDeployment(payload.toString(), entry);
                ...
                listener.getLogger()
                        .println("Successfully uploaded deployment to HCL Accelerate (" + logString + ").");
            } catch (Exception ex) {
                listener.error("Error uploading deployment data (" + logString + "): " + ex.getClass() + " - "
                        + ex.getMessage());
    ...
    public static class UploadDeploymentDescriptor extends BuildStepDescriptor<Builder> {
        ...
        public String getDisplayName() {
            return "HCL Accelerate - Upload Deployment to HCL Accelerate";
        }
        ...
        public ListBoxModel doFillInstanceBaseUrlItems(@QueryParameter String currentInstanceBaseUrl) {
            ...
            String all = "Upload Deployment to All HCL Accelerate Instances";

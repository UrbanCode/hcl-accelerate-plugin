...
public class UploadMetricsFile extends Builder implements SimpleBuildStep {
    ...
    throws AbortException, InterruptedException, IOException {
        ...
        for (Entry entry : finalEntriesList) {
            ...
            listener.getLogger()
                    .println("Uploading metric \"" + name + "\" to HCL Accelerate (" + logString + ").");
            if (!entry.isConfigured()) {
                listener.getLogger()
                        .println("Could not upload metric file to Accelerate as there is no configuration specified.");
                return;
            }
            ...
            if (!success) {
                listener.getLogger()
                        .println("Problem uploading metrics file to HCL Accelerate (" + logString + ").");
                ...
                listener.getLogger()
                        .println("Successfully uploaded metric file to HCL Accelerate (" + logString + ").");
            }
        }
    }
    ...
    public static class UploadMetricsFileDescriptor extends BuildStepDescriptor<Builder> {
        ...
        public String getDisplayName() {
            return "HCL Accelerate - Upload Metrics File to HCL Accelerate";
        }
        ...
        public ListBoxModel doFillInstanceBaseUrlItems(@QueryParameter String currentInstanceBaseUrl) {
            ...
            List<Entry> entries = Jenkins.getInstance().getDescriptorByType(DevOpsGlobalConfiguration.class)
                    .getEntries();
            String all = "Upload Metric File to All HCL Accelerate Instances";
